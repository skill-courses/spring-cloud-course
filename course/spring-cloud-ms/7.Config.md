...
spring:
  cloud:
    config:
      label: master #分支名称
      name: order-service-dev.yml  #配置文件名称
      profile: dev  #环境名  order-service-dev.yml 中的 dev
      discovery:
        enable: true
...
访问刚刚的url`http://localhost:8762/getConfig`,然后修改Git仓库里面配置的内容，可以观察到此时并没有重启服务，但是配置却生效啦！
